...
def build_parser(possible_tokens):
    ...
    @pg.production('statement : function_dec')
    def statement_to_func(p):
        return p[0]
...
###############################
    @pg.production('function_dec : HOW IZ I IDENTIFIER arguments MKAY newlines code_block IF U SAY SO type_declaration')
    def func_def(p):
        return Function([p[3].getstr() , p[4] , p[7] , p[12]])
        
    @pg.production('expression : FOUND YR expression')
    def found(p):
        return Found([p[2]])
    
    @pg.production('arguments : argument_declaration arguments')
    def func_def(p):
        return p[0] , p[1]
    ...
    @pg.production('argument_declaration : optional_an YR IDENTIFIER type_declaration')
    def func_def(p):
        return [p[2].getstr() , p[3]]
    ...
    @pg.production('expression : function_call')
    def expr_to_fun_call(p):
        return p[0]
    
    @pg.production('function_call : I IZ IDENTIFIER parameters MKAY')
    def fun_call(p):
        return FunctionCall((p[2].getstr() , p[3]))
    
    @pg.production('parameters : optional_an YR expression parameters')
    def params(p):
        if p[2] != None:
            return [p[2] , p[3]]
        else:
   ...
   # I IZ function_many YR 7 YR 'a' MKAY
   # I IZ function_zero MKAY
##############################
